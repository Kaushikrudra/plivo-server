...
app.post('/answer', async (req, res) => {
  ...
  // If caller (From) is a SIP URI or starts with agent naming conventions, it's an outbound call from an agent
  const isSipCaller = from.toLowerCase().startsWith('sip:') || from.toLowerCase().startsWith('zohoagent') || from.toLowerCase().startsWith('edwinagent');
  const isIncoming = !isSipCaller && ((direction.toLowerCase() === 'inbound') || (cleanTo === cleanPlivoNum && cleanPlivoNum !== ''));
